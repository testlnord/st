...
//  features2.add_feature(&FEntropyGrey);
 ...
 features3.add_feature(&FXDerivative);
 features3.add_feature(&FYDerivative);
  ...
  int imgn = 0;
   ...
   imgn = 0;
    ...
    for (int i = 0; i<goodtrainsamples; i++){
        img = dataset->getBadImg();
        if (boost1.predict(features1.generate_feature_vector(*img))>0)
            featureVector->push_back(features2.generate_feature_vector(*img));
        else
            i--;
        delete img;
    }
    ...
    classLabelResponses= Mat(featureVector->rows, 1, CV_32FC1,-1.0f);
    goodroi = classLabelResponses(Range(0,goodtrainsamples),Range::all());
    goodroi = Scalar(1.0f);
    badroi = classLabelResponses(Range(goodtrainsamples,classLabelResponses.rows),Range::all());
    badroi = Scalar(-1.0f);
...
//	cout << "DEBUG:   "<< currentNci<< endl;
   ...
   imgn = 0;
    ...
    for (int i = 0; i<goodtrainsamples; i++){
        img = dataset->getBadImg();
        if (boost1.predict(features1.generate_feature_vector(*img))>0)
            if (boost2.predict(features2.generate_feature_vector(*img))>0)
                featureVector->push_back(features3.generate_feature_vector(*img));
            else
                i--;
        else
            i--;
...
//    cout << "DEBUG:   "<< currentNci<< endl;
   ...
   cout << endl<< endl<<"REAL TESTING"<<endl;
	...
	while(img->data){
      ...
      while(s<100 && s < img->cols/5 && s < img->rows/5){
		while(y+s<img->rows){
		  while(x+s<img->cols){
			Rect current(x,y,s,s);
